...
public class GUIExperiment extends Experiment {
    ...
    private DoubleProperty smoothLossProperty = new SimpleDoubleProperty(1);
    private DoubleProperty noSamplesProperty = new SimpleDoubleProperty(0);
    private DoubleProperty samplesPerSecondProperty = new SimpleDoubleProperty(0);
    
    // integer values to be placed in top bar
    int noBatches=0;
    private IntegerProperty noBatchesProperty = new SimpleIntegerProperty(0);
    private IntegerProperty epochIdProperty = new SimpleIntegerProperty(0);
    
    //stop watch
    private StopWatch batchTime = new StopWatch(false); 
    private StopWatch displayAttributesTime = new StopWatch(true);
    ...
    public void trainAndSaveModel(Optimizer sgd, InputReader reader, int epochs, int activations) {
        ...
        infoMap.put("epochs",Integer.valueOf(epochs));
        infoMap.put("activations",Integer.valueOf(activations));
    ...
    public void addInfo(String label, Map<String,Object> infoMap) {
        Platform.runLater( () -> {
            ...
            infoAccordion.getPanes().add(
                new TitledPane(label, NodeUtil.createInfoView(infoMap))
            );
